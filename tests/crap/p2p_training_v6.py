...
def evaluate(model, dataloader, device, image_size=512, threshold=0.5, match_radius=25, nms_radius=0.03, find_best_threshold=True, debug=False):
    """Evaluate with optional threshold optimization."""
    model.eval()

    # First pass: collect all predictions and GTs
    all_preds, all_gts = [], []
    ...
    with torch.no_grad():
        for batch_idx, (images, targets) in enumerate(dataloader):
            ...
            if batch_idx == 0 and debug:
                ...
                mean_score = probs.mean().item()
                n_above_02 = (probs >= 0.2).sum().item()
                n_above_03 = (probs >= 0.3).sum().item()
                total_gt = sum(len(t['points']) for t in targets)
                print(f"  [Eval first batch] max={max_score:.3f} mean={mean_score:.3f} | "
                      f">0.2: {n_above_02}, >0.3: {n_above_03} | gt_points={total_gt}")

    # Print overall stats
    if debug:
        all_max = [p[0].max().item() for p in all_preds]
        all_mean = [p[0].mean().item() for p in all_preds]
        total_gt = sum(len(g) for g in all_gts)
        print(f"  [Eval overall] avg_max={np.mean(all_max):.3f} avg_mean={np.mean(all_mean):.3f} | "
              f"total_gt={total_gt} n_images={len(all_preds)}")
    ...
    def eval_at_threshold(thresh):
        ...
        for (scores, pts_norm), gt_pts in zip(all_preds, all_gts):
            ...
            total_preds_count += len(pred_pts)
        ...
        return {'precision': precision, 'recall': recall, 'f1': f1,
                ...
                'threshold': thresh, 'n_preds': total_preds_count}

    if not find_best_threshold:
        return eval_at_threshold(threshold)
    ...
    for thresh in thresholds:
        metrics = eval_at_threshold(thresh)
        if debug:
            print(f"  [Eval @{thresh:.1f}] P={metrics['precision']:.3f} R={metrics['recall']:.3f} F1={metrics['f1']:.3f} | "
                  f"preds={metrics['n_preds']} TP={metrics['tp']} FP={metrics['fp']} FN={metrics['fn']}")
...
class Trainer:
    def __init__(self, model, criterion, optimizer, scheduler, train_loader, val_loader, device, output_dir, config):
        ...
        # EMA for stable evaluation (faster decay = quicker convergence)
        self.ema = EMA(model, decay=0.99)  # 0.99 instead of 0.999
    ...
    def train(self, epochs, patience=20, unfreeze_epoch=None):
        ...
        for epoch in range(epochs):
            ...
            if self.val_loader:
                ...
                # Evaluate with debug on first few epochs
                debug = (epoch < 5 or epoch % 10 == 0)
                val_metrics = evaluate(self.model, self.val_loader, self.device,
                                       self.config['image_size'], 0.5,
                                       self.config['match_radius'],
                                       self.config.get('nms_radius', 0.03),
