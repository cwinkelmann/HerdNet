"""
Two-Stage Point Detector: Heatmap Proposals + Point Refinement (v3)
...
Architecture:
  ...
  Stage 2 - Point Refinement (IMPROVED but SIMPLE):
    - Single-scale ROI features (11×11) - proven to work
    - Global context (scene understanding) - NEW
    - Positional encoding (spatial relationships)
    - Deeper transformer (4 layers, 8 heads) - more capacity
    - Per-point classification (TP vs FP)
    - Per-point offset regression (sub-pixel refinement)

Key improvements over v1:
  1. Deeper transformer (2→4 layers, 4→8 heads, 2x→4x feedforward)
  2. Global context branch (see whole scene, not just local patch)
  3. Larger ROI (7→11)
  4. Increased match radius (0.05→0.08) for better training signal
  5. Keeps training stable (no hard negative mining, no multi-scale complexity)

Training:
  - Stage 1: MSE loss on heatmap
  - Stage 2: BCE + L1 loss on proposals matched to GT
"""
...
# =============================================================================
# STAGE 2: POINT REFINEMENT NETWORK (SIMPLIFIED WITH GLOBAL CONTEXT)
# =============================================================================

class PointRefinementNet(nn.Module):
    """
    Stage 2: Refine candidate points with global context.

    For each candidate:
    1. Extract local features via bilinear sampling (11×11 ROI)
    2. Add global context (entire image scene)
    3. Add positional encoding (spatial awareness)
    4. Apply deeper transformer (4 layers, 8 heads)
    ...
    def __init__(self, feat_dim: int, hidden_dim: int = 256,
        ...
        # ROI feature extractor (sample roi_size x roi_size around each point)
        ...
        # Point transformer - DEEPER than v1
    ...
    def extract_roi_features(self, feat: torch.Tensor, points: torch.Tensor) -> torch.Tensor:
        """
        Extract ROI features around each point.
    ...
    def forward(self, feat: torch.Tensor, points: torch.Tensor,
        ...
        # Extract local ROI features
        roi_feat = self.extract_roi_features(feat_proj, points)  # (B, N, hidden_dim)

        # Add global context - NEW!
        # This lets each point see the entire image, not just local patch
        ...
        # Combine: Local (ROI) + Global (scene) + Position (spatial)
...
class TwoStagePointDetector(nn.Module):
    ...
    2. Point-wise refinement with global context
    """
    ...
    def __init__(self, backbone: str = 'vit_large_patch16_dinov3.sat493m',
        ...
        # Stage 2: Point refinement with global context
...
class TwoStageLoss(nn.Module):
    ...
    def __init__(self, heatmap_sigma: float = 2.0,
                 ...
                 match_radius: float = 0.08, pos_weight: float = 3.0):
        super().__init__()
    ...
    def forward(self, outputs: Dict[str, torch.Tensor],
        ...
        # === Stage 2 Loss: Classification + Offset ===
        stage2_cls_loss = 0.0
        stage2_off_loss = 0.0
        n_matched = 0
        n_proposals = 0
        ...
        for b in range(B):
            ...
            # Classification loss on valid proposals
            valid = prop_mask[b]
            if valid.any():
...
def main():
    ...
    parser.add_argument('--output_dir', default='./outputs_twostage_v3')
