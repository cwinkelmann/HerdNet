...
Architecture:
  ...
  Stage 2 - Point Refinement (NEW):
    - For each candidate, extract local features via RoI
    - Point transformer: refine using local context
    - Per-point classification (true positive vs false positive)
    - Per-point offset regression (sub-pixel refinement)

Why this should beat HerdNet:
  1. Stage 1 catches most objects (high recall like HerdNet)
  2. Stage 2 filters false positives (better precision)
...
class PointRefinementNet(nn.Module):
    """
    Stage 2: Refine candidate points.

    For each candidate:
    1. Extract local features via bilinear sampling
    2. Apply small transformer for context
    3. Classify (TP vs FP) and regress offset
    """
    ...
    def extract_roi_features(self, feat: torch.Tensor, points: torch.Tensor) -> torch.Tensor:
        ...
        # Grid of offsets in normalized coordinates
        ...
        # points: (B, N, 2) -> (B, N, roi_size, roi_size, 2)
        ...
        # Reshape for grid_sample: (B, C, H, W) sampled at (B, N*roi*roi, 2)
    ...
    def forward(self, feat: torch.Tensor, points: torch.Tensor,
        ...
        roi_feat = self.extract_roi_features(feat, points)  # (B, N, hidden_dim)

        # Add positional encoding
        pos_feat = self.pos_encoder(points)  # (B, N, hidden_dim)
        roi_feat = roi_feat + pos_feat  # Add position information
        ...
        refined = self.transformer(roi_feat, src_key_padding_mask=attn_mask)

        # Predict
        cls_logits = self.cls_head(refined).squeeze(-1)  # (B, N)
        offsets = self.offset_head(refined)  # (B, N, 2)
        offsets = torch.tanh(offsets) * 0.1  # Limit offset to ±10% of image

        return {'cls_logits': cls_logits, 'offsets': offsets}
...
class TwoStageLoss(nn.Module):
    ...
    def __init__(self, heatmap_sigma: float = 2.0,
                 ...
                 match_radius: float = 0.05, pos_weight: float = 3.0):  # 5% of image = 25.6 pixels
        super().__init__()
    ...
    def forward(self, outputs: Dict[str, torch.Tensor],
        ...
        if self._step % 50 == 0:
            with torch.no_grad():
                ...
                print(
                    f"  [Step {self._step}] s1={stage1_loss.item():.4f} s2_cls={stage2_cls_loss.item() if isinstance(stage2_cls_loss, torch.Tensor) else stage2_cls_loss:.4f} "
...
def main():
    ...
    parser.add_argument('--output_dir', default='./outputs_twostage')
